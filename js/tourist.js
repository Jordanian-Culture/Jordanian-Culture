...
let imageArrayPetra = [
  ...
  {
    src: 'https://image.freepik.com/free-photo/person-front-temple-petra-jordan_143092-972.jpg',
    width: '320',
    height: '195'
  },
  {
...
let div = ['petra', 'wadiRum', 'deadSea', 'roman', 'citadel'];
...
function displayRandomImages( imageArray, div ) {
  ...
  let newImage = getRandomImage( 0, newArray.length - 1 );
...
displayRandomImages( imageArrayDeadSea, div[2] );
displayRandomImages( imageArrayRoman, div[3] );
displayRandomImages( imageArrayCitadel, div[4] );
...
// // let myIndex = Math.floor(Math.random()*x.length);
// // shuffle();
// // carousel();
...
function runder() {
  ...
  const parentElement = document.getElementById( 'newPlace' );
...
formElement.addEventListener( 'submit', function ( event ) {
  ...
  const nNewSectionTitle = event.target.newPlace.value;
  const nNewSectionText = event.target.newText.value;
